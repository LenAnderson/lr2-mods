...
init 0 python:
    ...
    def LENA_nearby_location_enabled(nearby_location):
        if nearby_location == mc.location:
            return 'You are here'
        return True
    def LENA_sort_location_list(loc_action):
        return re.sub(r'^(\{.*\})?\s*(.*)$', r'\2', loc_action.name.lower(), flags=re.DOTALL)
    def LENA_build_nearby_location_list():
        nearby_location_list = []
        current_region = [x for x in map_icons if mc.location in x.locations]
        if __builtin__.len(current_region) > 0:
            current_region = current_region[0]
            current_region_tooltips = current_region.get_tooltips()
            for loc in current_region.locations:
                if loc.visible or (not loc.hide_in_known_house_map):
                    if loc.name in current_region_tooltips:
                        loc_name = current_region_tooltips[loc.name][1].split('\n')
                        loc_name = ' '.join(loc_name[:-1]) + '\n' + loc_name[-1]
                        loc_tt = current_region_tooltips[loc.name][0]
                    else:
                        loc_name = loc.formal_name + ' (ERROR)'
                        loc_tt = ''
                    nearby_location_list.append(Action(
                        loc_name.strip(),
                        LENA_nearby_location_enabled,
                        "LENA_change_location",
                        loc,
                        requirement_args=loc,
                        menu_tooltip=loc_tt
                    ))
        nearby_location_list.sort(key=LENA_sort_location_list)
        nearby_location_list.insert(0, "Go Somewhere")
        return nearby_location_list
...
label LENA_change_location(new_location):
    $ mc.change_location(new_location)
    call change_location(new_location) from LENA_call_change_location #_return is the location returned from the map manager.
